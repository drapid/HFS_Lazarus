...
// strings array
  function  stringExists(const s: String; const a: array of String; isSorted: Boolean=FALSE): Boolean; OverLoad;
  function  stringExists(const s: UnicodeString; const a: array of UnicodeString; isSorted: Boolean=FALSE): Boolean; OverLoad;
  ...
  procedure removeStrings(const find: String; var a: TStringDynArray); OverLoad;
  procedure removeStrings(const find: String; var a: TUnicodeStringDynArray); OverLoad;
  function  removeString(var a: TUnicodeStringDynArray; idx: integer; l: Integer=1): Boolean; overload;
  function  removeString(const s: String; var a: TUnicodeStringDynArray; onlyOnce: Boolean=TRUE; ci: Boolean=TRUE; keepOrder: Boolean=TRUE): Boolean; overload;
  ...
  function  addArray(var dst: TMacroTableVal; src: array of UnicodeString): Integer; OverLoad;
  function  addArray(var dst: TUnicodeStringDynArray; src: array of UnicodeString; where:integer=-1; srcOfs:integer=0; srcLn:integer=-1): Integer; OverLoad;
  function  addArray(var dst: TUnicodeStringDynArray; src: array of String; where:integer=-1; srcOfs:integer=0; srcLn:integer=-1): Integer; OverLoad;
  function  removeArray(var src: TstringDynArray; toRemove:array of string):integer;
  function  UnicodeSplit(const separator, s: UnicodeString; nonQuoted: Boolean=FALSE): TUnicodeStringDynArray;
  ...
  function  join(const separator: String; ss: TStringDynArray): String; OverLoad;
  function  join(const separator: UnicodeString; ss: TUnicodeStringDynArray): UnicodeString; OverLoad;
  ...
  function  toSA(a: array of UnicodeString): TUnicodeStringDynArray; // this is just to have a way to typecast
  function  toMSA(a: array of UnicodeString): TMacroTableVal; // this is just to have a way to typecast
  function  addUniqueString(const s: String; var ss: TStringDynArray): Boolean; OverLoad;
  function  addUniqueString(const s: String; var ss: TUnicodeStringDynArray): Boolean; OverLoad;
  function  addString(const s: String; var ss: TStringDynArray): integer; OverLoad;
  function  addString(const s: UnicodeString; var ss: TUnicodeStringDynArray): Integer; OverLoad;
  function  replaceString(var ss: TStringDynArray; const old, new: String): Integer;
  function  popString(var ss: TstringDynArray): String; OverLoad;
  function  popString(var ss: TUnicodeStringDynArray): UnicodeString; OverLoad;
  procedure insertString(const s: String; idx: Integer; var ss: TStringDynArray); OverLoad;
  procedure insertString(const s: String; idx: Integer; var ss: TUnicodeStringDynArray); OverLoad;
  function  addUniqueArray(var a: TstringDynArray; b:array of string): Integer;
  procedure uniqueStrings(var a: TstringDynArray; ci:Boolean=TRUE); OverLoad;
  procedure uniqueStrings(var a: TUnicodeStringDynArray; ci:Boolean=TRUE); OverLoad;
  procedure sortArray(var a: TStringDynArray);
  function  sortArrayF(const a: TStringDynArray):TStringDynArray;
  function  idxOf(const s: String; a: array of string; isSorted:boolean=FALSE): Integer; OverLoad;
  function  idxOf(const s: UnicodeString; a: array of UnicodeString; isSorted:boolean=FALSE): Integer; OverLoad;
  ...
  function getStr(from, to_: PUnicodeChar): UnicodeString; OverLoad;
 {$IFDEF UNICODE}
 ...
 {$ENDIF UNICODE}
  ...
  function getSectionAt(p: pchar; out name: String): Boolean; OverLoad;
  function getSectionAt(p: PUnicodeChar; out name: UnicodeString): Boolean; OverLoad;
  ...
  function first(const a, b: UnicodeString): UnicodeString; overload;
...
//    function length():integer;
  ...
  TFastUStringAppend2 = class
   ...
   public
    ...
    function Insert(Index: Integer; const Value: UnicodeString): TFastUStringAppend2;
    function Remove(Index: Integer; len: Integer): TFastUStringAppend2;
    //function PosEx(const str: UnicodeString; pos: Integer): Integer;
    function PosEx(const str: UnicodeString; pos: SizeUInt): Integer;
    function SubStr(start: Integer; upTo: Integer=0): UnicodeString;
...
//    function Equals(fastUStringAppend2: TFastUStringAppend2): Boolean; reintroduce;
    function Equals(const Str: UnicodeString): Boolean; reintroduce;
...
function TFastUStringAppend2.Equals(const Str: UnicodeString): Boolean;
begin
  Result := //(StringBuilder <> nil) and
       (Self.Length = System.Length(Str)) and
    ((Length = 0) or
...
function TFastUStringAppend2.PosEx(const str: UnicodeString; pos: SizeUInt): Integer;
begin
  //Result := StrUtils.PosEx(str, Self.FData, pos);
  Result := StrUtils.posEx(str, Self.FData, pos);
  if (System.Length(str) + Result + -1) > n then
    Result := 0;
end;

function TFastUStringAppend2.SubStr(start: Integer; upTo: Integer=0): UnicodeString;
begin
  if (start > n) then
    Result := ''
   else
...
function TFastUStringAppend2.Insert(Index: Integer; const Value: UnicodeString): TFastUStringAppend2;
var
  OldLength: Integer;
  Len2: Integer;
begin
  ...
  if len2 > 0 then
    begin
      n := n + len2;
      if n > system.Length(FData) then
        begin
          SetLength(FData, n + incStep);
        end;

      if (OldLength > Index) then
        Move(FData[Index], FData[Index + len2], (OldLength - Index+1) * SizeOf(UnicodeChar));
      //Move(Value[Low(string)], FData[Index], len2 * SizeOf(Char));
      Move(Value[1], FData[Index], len2 * SizeOf(UnicodeChar));
    end;
  Result := Self;
end;
...
begin
  if len > 0 then
    begin
      ...
      if cl > 0 then
      {$IFDEF FPC}
        Move(FData[Index+len2], FData[Index], cl * SizeOf(UnicodeChar));
      {$ELSE ~FPC}
  ...
  if common > 0 then
   begin
    if (upTo = n) then
      // Just append
      begin
        if ((newL>oldL)) and (system.Length(FData) < (n + newL - oldL)) then
...
//        oldL := upTo-start+1;
  ...
  if needMove then
    begin
      if surplus > 0 then
        Self.Remove(start+system.Length(ss), surplus)
...
begin
 ...
 {$IFDEF FMX}
          if default = TColorRec.Null then
 {$ELSE ~FMX}
          if default = clNone then
 {$ENDIF ~FMX}
...
function AnsiIndexText(const AText: UnicodeString; const AValues: array of UnicodeString): Integer; OverLoad;
begin
...
function stringExists(const s: UnicodeString; const a: array of UnicodeString; isSorted: Boolean=FALSE): Boolean;
...
function addArray(var dst: TUnicodeStringDynArray; src: array of UnicodeString; where:integer=-1; srcOfs:integer=0; srcLn:integer=-1):integer;
var
  i, l:integer;
begin
  l:=length(dst);
...
function addArray(var dst: TUnicodeStringDynArray; src: array of String; where:integer=-1; srcOfs:integer=0; srcLn:integer=-1):integer;
var
  i, l:integer;
begin
  l:=length(dst);
...
procedure removeStrings(const find: String; var a: TUnicodeStringDynArray);
var
  i, l: integer;
begin
  repeat
...
function popString(var ss: TUnicodeStringDynArray): UnicodeString;
begin
  result:='';
  if ss = NIL then
    exit;
...
function addString(const s: UnicodeString; var ss: TUnicodeStringDynArray): Integer;
begin
  result := length(ss);
  addArray(ss, [s], result)
end; // addString
...
procedure insertString(const s: String; idx: Integer; var ss: TUnicodeStringDynArray);
begin
  addArray(ss, [s], idx)
end;
...
function removeString(const s: String; var a: TStringDynArray; onlyOnce: Boolean=TRUE; ci: Boolean=TRUE; keepOrder: Boolean=TRUE): Boolean;
var
  i, lessen:integer;
begin
result:=FALSE;
...
function removeString(const s: String; var a: TUnicodeStringDynArray; onlyOnce: Boolean=TRUE; ci: Boolean=TRUE; keepOrder: Boolean=TRUE): Boolean;
var
  i, lessen:integer;
begin
result:=FALSE;
...
function removeString(var a: TUnicodeStringDynArray; idx: integer; l:integer=1): Boolean;
begin
  result:=FALSE;
...
function getStr(from, to_: PUnicodeChar): UnicodeString;
var
  l: integer;
begin
  result:='';
...
function first(const a, b: UnicodeString): UnicodeString;
begin
...
var
  ...
  procedure drawSample(sample: int64; clr: TAlphaColor);
  var
    a: Integer;
  begin
    a := (sample*h div maxV);
    cnv.DrawLine(PointF(r.left+i, r.bottom), PointF(r.Left+i, r.Bottom - 1 - a), 1, TStrokeBrush.Create(TBrushKind.Solid, clr));
  end; // drawSample

  function getColor(idx: integer; def:Tcolor):Tcolor;
  begin
    if (length(colors) <= idx) or (colors[idx] = TColorRec.SysDefault) then
      result := def
     else
      result := colors[idx]
  end; // getColor
...
begin
  ...
  r.Right := cnv.Width;
  r.Bottom := cnv.Height;
  if cnv.BeginScene() then
   begin
    cnv.Clear(getColor(0, TColorRec.Black));
    // draw grid
    i := 0;
    clr := getColor(1, rgb(0,0,120));
    while i < cnv.Width do
      begin
        cnv.DrawLine(PointF(i, r.top), PointF(i, r.Bottom), 1, TStrokeBrush.Create(TBrushKind.Solid, clr));
        inc(i,10);
      end;
    i := Round(r.bottom);
    while i > r.top do
      begin
        cnv.DrawLine(PointF(r.left, i), PointF(r.right, i), 1, TStrokeBrush.Create(TBrushKind.Solid, clr));
        dec(i,10);
      end;

    maxV := max(graph.maxV, 1);
    h := Round(r.bottom-r.top-1);
    // draw graph
    if r.Right > r.left then
      begin
        sI := Min(Trunc(r.Right-r.left)-1, Length(graph.samplesOut)-1);
        clr := getColor(2, TColorRec.Fuchsia);
        for i:=0 to sI do
          drawSample(graph.samplesOut[i], clr);
        clr := getColor(3, TColorRec.Yellow);
        for i:=0 to sI do
          drawSample(graph.samplesIn[i], clr);
      end;

    // text
  //  cnv.Font.color := getColor(4, clLtGray);
    cnv.Font.Family := 'Small Fonts';
    cnv.font.size := 7;

    top := (graph.maxV/1000)*safeDiv(10.0, graph.rate);
    s := format(TOP_SPEED+':'+MSG_SPEED_KBS+'    ---    %d kbps', [top, round(top*8)]);
  //  w := cnv.TextWidth(s);
    rt.Left := r.Right - cnv.TextWidth(s) - 20 * cnv.Scale;
    rt.Top := 3 * cnv.Scale;
    rt.Bottom := r.Bottom;

    cnv.FillText(rt, s, false, 1, [], TTextAlign.Leading, TTextAlign.Leading);
    if assigned(globalLimiter) and (globalLimiter.maxSpeed < MAXINT) then
      begin
        rt.Left := r.right-(180-25)*cnv.Scale;
        rt.Top := 15 * cnv.Scale;

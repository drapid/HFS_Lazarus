...
uses
  iniFiles, types,
  srvConst, srvClassesLib, serverLib;

  function getVarSpace(md: TmacroData; var varname: String): THashedStringList;
  function cbMacros(fs: TFileServer; const fullMacro: UnicodeString; pars: TPars2; cbData: PMacroData): UnicodeString;
...
//  unnamedPars: integer; // this is a guessing of the number of unnamed parameters. just guessing because there's no true distinction between a parameter "value" named "key", and parameter "key=value"
  ...
  begin
    result := '<div class=macroerror>macro error: '+name+nonEmptyConcat('<br>',msg)+'</div>'
  end;
...
//    add2log('WARNING, deprecated macro: '+first(what, name)+nonEmptyConcat(' - Use instead: ',instead), NIL, clRed)
  ...
  begin
    result:=(s='') or (strToFloatDef(s,1) = 0)
  end;

  function isTrue(const s: String): boolean; inline;
  begin
    result:=not isFalse(s)
  end;
  ...
  function getVar(const varname: String): String; overload;
  var
    vn: String;
  begin
    vn := varname;
    result := getVarSpace(vn).values[vn]
  end;
  ...
  var
    ...
    function getAttr(const name: String; a: TfileAttribute): Boolean;
    begin
      result := w = name;
      if result then
        trueIf(a in f.flags);
...
//      s := pars.parEx('newname');
  ...
  begin
    result := '';
    space := getVarSpace(st);
    if not satisfied(space) then
      exit;
    // set the table variable as text
    v := par(1);
    // access the table object
    i := space.indexOfName(st);
    if i < 0 then
      begin
        h:=ThashedStringList.create();
        space.AddPair(st, v, h);
      end
     else
      h := space.objects[i] as THashedStringList;
    // fill the object
  ...
  begin
    ...
    try
      ...
      for i:=0 to space.count-1 do
        if ansiStartsText(d, space.names[i]) then
          begin
          if fs.length > 0 then
            fs.append(sep);
          if values then
  ...
  begin
    trueIf(assigned(f) and f.hasRecursive(FA_ARCHIVABLE) or (f = NIL) and md.archiveAvailable)
  end;
...
//    i: integer;
  ...
  // search for the symbol in the translation table
    ...
    if ansiStartsText('%sym-', name) then // legacy: surpassed by {.section.}
      ...
      result:=intToStr(fs.getConnectionsCount)
      ...
      result := floatToStrF(fs.getSpeed(ST_OUTCOME)/1000, ffFixed, 7,2)
    else if name = '%speed-in%' then
      result:=floatToStrF(fs.getSpeed(ST_INCOME)/1000, ffFixed, 7,2)
    else if name = '%total-out%' then
      result:=smartSize(outTotalOfs + fs.getBytes(ST_OUTCOME))
    else if name = '%total-in%' then
      result:=smartSize(inTotalOfs + fs.getBytes(ST_INCOME))
    ...
    if not lFound and assigned(md.f) then
      if name = '%item-name%' then
        ...
        result := floatToStr(md.f.getTime(True))
      else if name = '%item-modified-dt%' then
        result := floatToStr(md.f.getTime(False))
      // these twos are actually redundant, {.time||when=%item-added-dt%.}
      else if name = '%item-added%' then
        result := datetimeToStr(md.f.getTime(True))
      else if name = '%item-modified%' then
        begin
          if md.f.getTime(False) = 0 then
            Result := 'error'
           else
            Result := datetimeToStr(md.f.getTime(False))
        end
...
//      if (spDisableMacros in sp) then
      if not(spEnableMacros in sp) then
        exit(fullMacro);
      ...
      if not Assigned(pars) or (pars.count = 0) then
        exit;
...
//          if fs.macroFuncs.TryGetValue(name, ff) then
          if fs.TryGetMacroFunc(name, ff) then
            begin
      ...
      if name = 'time' then
        begin
          ...
          if s = 'y' then
            ...
            begin
              tempStr := Result;
              datetimeToString(tempStr, first(s,'c'), r );
              Result := tempStr;
            end;
        end
       else
  ...
  //        result:=noMacrosAllowed(UTF8toAnsi(p))
      ...
      if name = 'get' then
        try
          if p = 'can recur' then
//             trueIf(lpRecurListing in lp)// mainFrm.recursiveListingChk.Checked)
             trueIf(spRecursiveListing in SP)
          ...
          else if p = 'protocolon' then result := fs.protoColon
        ...
        begin
          if satisfied(md.cd) then
            try
              ...
              md.cd.CreateLimiter(round(parF(0)*1000));
              fs.addLimiter(md.cd.limiter);
              result:='';
             except
              fs.removeLimiter(md.cd.limiter);
              md.cd.RemoveLimiter;
              result:='';
            end;
        end
          ...
          if satisfied(md.cd) then
           begin
            ...
            if ansiStartsText('Content-Type:', p) then
              ...
              md.cd.conn.addHeader(String(p), isTrue(par('overwrite',true,'1')));
           end;
        end
       else
        ...
        begin
          ...
          if (result > '') and not stopOnMacroRename then // should we stop recursion?
            try
              ...
              fs.runEventScript('on macro rename', toSA(['%old-name%',p,'%new-name%',par(1)]), md.cd);
             finally

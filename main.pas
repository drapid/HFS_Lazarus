...
//    function  getVFSJZ(node: TFileNode=NIL): RawByteString;
 ...
 public
    ...
    function  addFile(f: TFile; parent: TFileNode=NIL; skipComment: Boolean=FALSE): TFile; OverLoad;
...
resourcestring
  MSG_TO_EXPERT = 'Switch to expert mode.';
...
resourcestring
  MSG_UNINSTALL_WARN = 'Delete HFS and all settings?';
resourcestring
...
resourcestring
  MSG_ICONS_ADDED = '%d new icons added';
resourcestring
  MSG_DDNS_DISABLED = 'Dynamic DNS updater disabled';
resourcestring
...
resourcestring
  MSG_UPL_HOWTO = '1. Add a folder (choose "real folder")'
  +#13
...
begin
  result := NIL;
  {$IFDEF FMX}
  n := filesBox.ItemByPoint(p.X, p.Y);
  if (n = NIL) or n.Data.IsEmpty then
    exit;
  result := n.data.AsType<tfile>;
  {$ELSE ~FMX}
...
procedure updateMenuSpeed(menu: TMenuItem; const lab: String; v: real);
begin
...
function expandAccountByLink(a: Paccount; noGroups: boolean=TRUE): TUnicodeStringDynArray;
var
  i: integer;
begin
  result := NIL;
...
function expandAccountsByLink(users: TStringDynArray; noGroups:boolean=TRUE): TUnicodeStringDynArray;
var
  i: integer;
begin
  result := NIL;
      ...
      begin
        ...
        if Length(Files) > 0 then
          for i:=0 to Length(Files)-1 do
            addFile(Tfile.create(fileSrv, files[i]), filesBox.Selected, Length(Files)<>1 );
...
begin
  ...
  if selectedFile.isFolder then
    ...
    begin
      fn := res;
      done := PromptForFileName(fn, '', '', MSG_EDIT_RES);
      res := fn;
    end;
  if done then
    VFSmodified := TRUE;
       ...
       for i:=0 to count-1 do
        with items[i] do
         if caption = labl then
           begin
            width := MAX(strToIntDef(s, width), LVSCW_AUTOSIZE_USEHEADER);
            break;
           end;
      end;
